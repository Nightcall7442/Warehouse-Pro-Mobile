...
describe("напоминания о визитах", () => {
  ...
  it("эффект зависит от числа, а не от массива планов", () => {
    ...
    // t из useT — функция языка: меняется только при переключении языка,
    // а не при каждом ответе запроса, и массива планов в зависимостях нет.
    expect(src).toMatch(/[isAuthenticated, plannedCount(, t)?]/);
  });
});
