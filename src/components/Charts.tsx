...
export function DonutChart({ segments, size = 140, strokeWidth = 20, centerLabel, centerSublabel }: {
  ...
  const arcs = segments.reduce<{ items: Array<typeof segments[number] & { pct: number; offset: number; rotation: number }>; acc: number }>(
    (result, seg) => {
      const pct = seg.value / total;
      const offset = circumference * (1 - pct);
      const rotation = (result.acc / total) * 360 - 90;
      return { items: [...result.items, { ...seg, pct, offset, rotation }], acc: result.acc + seg.value };
    },
    { items: [], acc: 0 },
  ).items;
